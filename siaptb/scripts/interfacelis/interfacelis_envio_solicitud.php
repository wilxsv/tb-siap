<?php
    ...
    $localPath = realpath(dirname(__FILE__)).'/../../../logs/interfacelis_envio_solicitud_'.date("y-m")."_";
    ...
    try {
        ...
        if( $response->success ) {
            ...
            if( $token !== null && $token !== '' ) {
                $response = $session->get('app.php/api/laboratorio/solicitud/equipoautomatizado/send?token='.$token);

                if( $response->success ) {
                    $responseData = $response->body;
                    var_dump("---------EQUIPO_AUTOMATIZADO_SEND---------".$now->format('d/m/Y H:i:s')."----------");
                    var_dump($responseData);
                    ...
                    file_put_contents($localPath.'error001.log', "------------------------------------".$now->format('d/m/Y H:i:s')."------------------------------------\n", FILE_APPEND);
                    file_put_contents($localPath.'error001.log', "----------------------------------EQUIPO_AUTOMATIZADO_SEND---------------------------------\n", FILE_APPEND);
                    file_put_contents($localPath.'error001.log', $response, FILE_APPEND);
                    file_put_contents($localPath.'error001.log', "\n", FILE_APPEND);
                }
            } else {
        ...
        file_put_contents($localPath.'error001.log', "------------------------------------".$now->format('d/m/Y H:i:s')."------------------------------------\n", FILE_APPEND);
        file_put_contents($localPath.'error001.log', "----------------------------------EXCEPCION---------------------------------\n", FILE_APPEND);
