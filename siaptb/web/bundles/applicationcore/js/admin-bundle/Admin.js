...
//        Admin.setup_list_modal(subject);
    ...
    setup_collection_buttons: function(subject) {
        ...
        jQuery(subject).on('click', '.sonata-collection-add', function(event) {
            ...
            //setting collectionCount
            var collectionCount = 0;
            if( container.find('.sonata-collection-row').length > 0 ) {

                var collectionFormGroup = container.find('.sonata-collection-row').last().find('.form-group').attr('id');
                var countRegex = collectionFormGroup === undefined ? new RegExp(container.attr('id')+'_','g') : new RegExp('sonata-ba-field-container-'+container.attr('id')+'_','g');
                var collectionCount = collectionFormGroup === undefined ? container.find('.sonata-collection-row').last().find('.form-group div div').attr('id').replace(countRegex, '') : container.find('.sonata-collection-row').last().find('.form-group').attr('id').replace(countRegex, '');

                //var countRegex = new RegExp(container.attr('id')+'_','g');
                //collectionCount = container.find('.sonata-collection-row').last().find('div.control-group div.controls div').attr('id').replace(countRegex, '');
                collectionCount = parseInt(collectionCount) + 1;
            }

            // Set field id
            var idRegexp = new RegExp(container.attr('id')+'_'+protoName,'g');
            proto = proto.replace(idRegexp, container.attr('id')+'_'+collectionCount);

            // Set field name
            var parts = container.attr('id').split('_');
            var nameRegexp = new RegExp(parts[parts.length-1]+'\\]\\['+protoName,'g');
            proto = proto.replace(nameRegexp, parts[parts.length-1]+']['+collectionCount);
            jQuery(proto)
                .insertBefore(jQuery(this).parent())
                .trigger('sonata-admin-append-form-element')
            ;

            jQuery(this).trigger('sonata-collection-item-added', [ container.attr('id') , collectionCount ]);
        });

        jQuery(subject).on('click', '.sonata-collection-delete', function(event) {
            Admin.stopEvent(event);
            var container           = jQuery(this).closest('[data-prototype]');
            var row                 = jQuery(this).closest('.sonata-collection-row');
            var collectionFormGroup = row.find('.form-group').attr('id');
            var patternRegex        = collectionFormGroup === undefined ? container.attr('id')+'_' : 'sonata-ba-field-container-'+container.attr('id')+'_';
            var replaceRegex        = new RegExp(patternRegex,'g');
            var rowId               = collectionFormGroup === undefined ? row.find('.form-group div div').attr('id') : row.find('.form-group').attr('id');
            var index               = rowId.replace(replaceRegex, '');

            jQuery(this).trigger('sonata-collection-item-deleted', [container.attr('id'), index]);
