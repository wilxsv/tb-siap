...
class DateTimeFormatter
{
     ...
     * @return string
     */
    public function formatDiff(Datetime $from, Datetime $to, $parameters = array())
    {
        $diffMsg = '';
        $index = 0;
        $firtsMessage = true;
        ...
        foreach ($units as $attribute => $unit) {
            ...
            if( count($parameters) === 0 ) {
                if (0 !== $count) {
                    return $this->doGetDiffMessage($count, $diff->invert, $unit, $firtsMessage);
                }
            } else {
                if( in_array($attribute, $parameters) ) {
                    if (0 !== $count) {
                        $diffMsg .= ($index === 0 ? '' : ' ').$this->doGetDiffMessage($count, $diff->invert, $unit, $firtsMessage);
        ...
        if($index === 0) {
            $diffMsg = $this->getEmptyDiffMessage();
        }
